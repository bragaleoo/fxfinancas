...
import { Categoria, TipoParcela, Proposta } from '../types';
import { 
  Save, 
  Upload, 
  Loader2, 
...
export default function EditarProposta() {
  ...
  const [existingFiles, setExistingFiles] = useState<{ numero_parcela: number, url_arquivo: string, status: 'pendente' | 'paga' | 'inadimplente' }[]>([]);
  ...
  async function fetchProposta() {
    try {
      ...
      setExistingFiles(comprovantes || []);
      
      // Initialize installment status from existing data
      const initialStatus: { [key: number]: 'pendente' | 'paga' | 'inadimplente' } = { 1: 'pendente', 2: 'pendente', 3: 'pendente', 4: 'pendente' };
      comprovantes?.forEach((c: any) => {
        initialStatus[c.numero_parcela] = c.status || 'pendente';
      });
      setInstallmentStatus(initialStatus);

      // Format currency values for the form
      const formattedProposta = {
        ...proposta,
        valor_consorcio: formatCurrency((proposta.valor_consorcio * 100).toString()),
        valor_parcela: formatCurrency((proposta.valor_parcela * 100).toString())
      };

      reset(formattedProposta);
    } catch (error: any) {
  ...
  const onSubmit: SubmitHandler<PropostaFormData> = async (data) => {
    ...
    try {
      ...
      if (propostaError) {
        throw new Error('Erro ao atualizar proposta: ' + propostaError.message);
      }

      // 2. Upload New Files and Update/Insert Comprovantes
      try {
        ...
        const uploadPromises = installmentNums.map(async (num) => {
          ...
          const existing = existingFiles.find(f => f.numero_parcela === num);

          // If no file and no change in status (or no existing record), skip
          if (!file && existing && existing.status === status) return null;
          if (!file && !existing && status === 'pendente') return null;

          let publicUrl = existing?.url_arquivo || '';

          if (file) {
            const typedFile = file as File;
            const fileExt = typedFile.name.split('.').pop();
            const fileName = `${id}_parcela_${num}_${Math.random()}.${fileExt}`;
          ...
          if (existing) {
            return supabase
              .from('comprovantes_parcelas')
              .update({ url_arquivo: publicUrl, status: status })
              .eq('proposta_id', id)
              .eq('numero_parcela', num);
          } else {
            return supabase
              .from('comprovantes_parcelas')
              .insert([{
        ...
        toast.warning('Proposta atualizada, mas houve um problema com os comprovantes.');
      }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-zinc-400">Atualize os dados da proposta selecionada.</p>
          </div>
        </div>
        <img 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-8">
            <section className="glass-card p-6 space-y-6 sticky top-8">
              ...
              <p className="text-xs text-zinc-500">Atualize os comprovantes das parcelas 1 a 4.</p>
                          ...
                          {files[num] ? (
                            ...
                            <div className="text-center">
                              ...
                              <span className="text-[10px] text-zinc-500 uppercase font-bold">
                                {existing ? 'Substituir Comprovante' : 'Upload Comprovante'}
                              </span>
                            </div>
                          )}
                        </div>
